...
# Some packages may need to be installed from Bioconductor if not already present in local library
## Use the following with the package name in quotations to install packages from Bioconductor
BiocManager::install()
...
list.params <- Create.proc.params(
  ...
  data.frame(
    ...
    # 2nd metadata column
    "Region" = as.factor(ifelse(grepl("R1",basename(list.files("data/"))),"R1","R2")),
    # 3rd metadata column (add/remove columns as needed)
    "Time" = as.factor(ifelse(grepl("timeA",basename(list.files("data/"))),"timeA","timeB"))
    )
  )
...
```{r cell-pred, echo=F, fig.align= "center", out.width="90%", results=T, message=F, warning=F,eval=FALSE}
...
```{r mark-gene, echo=F, fig.align= "center", out.width="90%", results=T, message=F, warning=F,eval=FALSE}
...
```{r reclus, echo=F, fig.align= "center", out.width="90%", results=T, message=F, warning=F,eval=FALSE}
...
```{r dgea, echo=F, fig.align= "center", out.width="90%", results=T, message=F, warning=F,eval=FALSE}
...
```{r dgea-vis, echo=F, fig.align= "center", out.width="90%", results=T, message=F, warning=F,eval=FALSE}
...
### Coming Soon: Gene Ontology/Custom Gene Set Enrichment Analysis

```{r go-enrich, echo=F, fig.align= "center", out.width="90%", results=F, message=F, warning=F,eval=FALSE}
