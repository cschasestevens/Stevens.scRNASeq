...
#'
#' @param l.deg A dgea.output data frame created by sc.DGEA(). See sc.DGEA function documentation for details.
#' @param en.type Should enrichment statistics be calculated for GO terms or for a custom gene set list?
...
#' @param cstm.list Only used when conducting enrichment analysis based on a custom gene set list. 
...
#'
#' @export
sc.enrichment <- function(l.deg,en.type,cstm.list,parl,core.perc) {
  ...
  if(en.type == "cstm" & missing(cstm.list)){print("Error: must provide a custom gene set list!")}
  if(en.type == "cstm" & !missing(cstm.list)) {
    # params
    ## l.deg
    l1 <- l.deg
    ## cstm.list
...
#'
#' @param l.enr A data frame containing enrichment results generated by sc.enrichment().
#' @param en.type Are the provided enrichment results based on the Gene Ontology database or a custom gene set? Type "GO"
